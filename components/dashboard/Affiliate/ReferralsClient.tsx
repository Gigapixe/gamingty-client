...
export default function ReferralsClient() {
  ...
  const fetchReferralCodeAndStats = async () => {
    ...
    try {
      ...
      else toast.error(codeRes.message || "Failed to fetch referral code.");
    } catch (err: any) {
      toast.error(err?.message || "Error fetching referral code.");
    } finally {
    ...
    try {
      ...
      if (statsRes.status === "success") {
        ...
        toast.error(statsRes.message || "Failed to fetch referral stats.");
      }
    } catch (err: any) {
      toast.error(err?.message || "Error fetching referral stats.");
    } finally {
  ...
  const fetchReferralListData = async () => {
    ...
    try {
      ...
      if (listRes.status === "success" && listRes.data) {
        ...
        throw new Error(listRes.message || "Failed to fetch referral list.");
      }
    } catch (err: any) {
      setReferralError(err?.message || "Error fetching referral list.");
      setReferralList([]);
    } finally {
  ...
  const fetchCommissionHistoryData = async () => {
    ...
    try {
      ...
      if (historyRes.status === "success" && historyRes.data) {
        ...
        throw new Error(
          historyRes.message || "Failed to fetch commission history."
        );
      }
    } catch (err: any) {
      setCommissionHistoryError(
        err?.message || "Error fetching commission history."
      );
    ...
    () => [
      {
        ...
        width: "35%",
        sortable: true,
        renderCell: (row: any) => row.notes || "Commission from referral",
      },
      {
        header: "Date",
        accessor: "createdAt",
      ...
      },
      {
        ...
        renderCell: (row: any) => {
          ...
          return dayjs().isAfter(dayjs(clearanceUntil))
            ? "Cleared"
            : `Clears: ${dayjs(clearanceUntil).format("MMM D")}`;
        },
      },
    ],
    []
  ...
  return (
    <>
      <div className="space-y-8">
        ...
        <ReferralTableCard
          title={`My Referred Users (${
            loadingReferralList ? "..." : referralTotalDocs
          })`}
          ...
          emptyState={
            <EmptyState
              title="You have no referred users yet."
              subtitle="Share your link to get started and they'll appear here!"
            />
          }
          pagination={{
        ...
        <CommissionTableCard
          ...
          emptyState={
            <EmptyState
              title="You have no commission history yet."
              subtitle="Share your referral link. When friends sign up using it and complete the requirements, you'll earn rewards!"
            />
          }
          pagination={{
